...
### Added
- **Added a guided first-run setup wizard.** A first-time install with zero machines configured now opens a 3-step onboarding modal automatically (welcome -> connect your first machine -> done), instead of only a static empty-state hint. Step 2 reuses the existing Settings → Machines add-machine form and its test-connect logic directly (the same singleton form node is reparented into the wizard for that step, then moved back), rather than duplicating any of it, and offers a "I don't have a machine yet, show me demo data" shortcut that seeds demo data and skips to the done step. "Later" (and closing before the done step) never marks the wizard complete, so it reappears on the next launch until either a machine gets added or the wizard is seen through to the end; a "Restart setup tour" control in Settings → Machines reopens it on demand without touching that completion state. The existing "machine configured but unreachable" banner/panel (`components/onboarding.js`) is unchanged — it covers a separate case. Closes #744
- **Shot-import progress now pushes over a new real-time SSE connection (`GET /api/events`) instead of only refreshing every 30s.** The progress bar under the sidebar's "Shots (N)" header now advances per-shot instead of jumping in large batches, and the completion toast fires directly off the backend's own success/failure result instead of being inferred from an entry disappearing between two polls — the structural root cause behind #731/#734's toast reliability patches. `lib/sync.js`'s backfill loop now tracks a `loopOk` flag and reports it explicitly. The existing 30s `/api/status` polling logic is kept as an automatic fallback (never removed) for whenever the SSE connection can't be established at all (e.g. no HA Ingress/Companion App precedent existed for this yet) — a session that never sees SSE connect keeps working exactly as before, silently. `EventSource` can't send custom headers, so this one route (only this one) also accepts the API token as a `?token=` query param alongside the existing header, mirroring the existing `GET /api/token` "reaching the port is the boundary, not the token" precedent. Existing REST endpoints (`/api/status`, `/api/live/data`, `/api/preheat`) are unchanged — other GLP repos may still consume them directly. **Review follow-up: a failed backfill now shows its own "Import failed" toast over the SSE path too**, instead of silently showing nothing (only the completion toast existed before). **Live-testing follow-up (#738): the progress bar still updated in blocks over HA Ingress** — Ingress's nginx reverse proxy buffers streamed responses by default, holding the SSE stream until enough data accumulated instead of flushing each event immediately; the route now sends `X-Accel-Buffering: no` to opt out of that buffering. **Live-testing follow-up (#740): that alone wasn't enough** — over HA Ingress the connection still never opened in real time (confirmed unrelated to any external reverse proxy: identical behavior with and without one in front of Ingress), while the add-on's own direct port streamed correctly the whole time. The route now also disables Nagle's algorithm on the response socket and sends an initial ~2KB padding comment (ignored by `EventSource`, forces a flush past whichever layer buffers by a size threshold) before the first real event. Closes #735, #738, #740
- **Settings → Machines' "Test connection" button now saves the machine first, then tests it** — a brand-new (not-yet-saved) machine can be tested with one click instead of hitting a "save first" error. The result is shown inline; the form stays open so a bad host can be fixed and tested again in place (only the explicit "Save" button closes the form). The separate "Save & test" button introduced in an earlier unreleased change is gone again, merged into this one. **Live-testing follow-up (#731): that implicit save no longer triggers a shot import** — only an explicit "Save" click does (see the next entry); "Test connection" now saves with a `?sync=0` marker the server checks before firing the catch-up sync, so testing a connection never has an import as a side effect. **Live-testing follow-up (#733): an earlier version of this also auto-closed the form after testing (matching Save's behavior) — reverted, since that turned out to be confusing for a test action.** Closes #729, #731, #733
- **Saving any machine (not just a default-host change) now triggers a catch-up shot sync for it.** Previously, only a default machine's host change kicked off `syncShots()`; saving a newly configured *non-default* machine did nothing, leaving its shot history to wait for the next scheduled sync tick. Every successful *explicit* machine save now fires a sync immediately: `syncShots()` for the default machine, `syncMachineShots()` for any other one. Closes #729
- **New machines with a large backlog of unsynced shots now show an import progress indicator** (e.g. "Import 12/40") on its own row under the sidebar's "Shots (N)" header, with a short toast once the backfill finishes — previously squeezed inline into the header next to the flap-board odometer (#731) and gave no completion feedback at all. Only shown once a backfill has more than 5 shots to fetch; disappears automatically when it finishes. **Live-testing follow-up (#733): the completion toast could be missed entirely if the tab was backgrounded for the whole import** (the 30s status poll gets throttled while hidden) — the app now forces an immediate status refresh the moment the tab becomes visible again, so a still-running import is caught right away instead of waiting out a throttled poll. Closes #729, #731, #733
- **Settings → Machines' per-machine shot count could get stuck showing 0 for the rest of the session**, even on a machine with a full shot history — the machines list was rendered from the app's in-memory shot list before that list had actually finished loading on startup, and nothing ever re-rendered it afterwards. Fixed by re-rendering the list once both the machines and the shots are guaranteed loaded. Closes #733

### Fixed
- **A newly-configured or just-reconnected machine could sit with an empty Shots list for up to the full `sync_interval` (default 5 minutes) even though it was already reachable.** Two gaps combined: (1) `lib/poll.js`'s `pollViaGaggiuinoStatus()` polls `/api/system/status` every second and always knew `state.machineReachable` in real time, but nothing hooked that back into triggering a sync — the only existing catch-up mechanism, `syncSoonAfterPowerOn()`, only fires from a configured HA switch entity's off→on transition, which most installs don't have; (2) `routes/machines.js`'s `POST /api/machines`/`PUT /api/machines/:id` handlers saved a new/changed host but never triggered a sync at all, so a user who just entered their machine's host via Settings → Machines had to wait for whichever came first: the live-poll noticing (still gated by #1) or the next scheduled tick. `pollViaGaggiuinoStatus()` now fires an additional, fire-and-forget `syncShots()` the moment a poll recovers from unreachable to reachable (a genuine false→true transition, not just "still reachable") *and* either the last sync errored or none has ever succeeded — covers connectivity flapping on an already-configured host. Separately, saving the default machine's host via either machine route now triggers `syncShots()` directly and immediately, covering the more common "I just told you the host, go check now" case without waiting on the poll loop at all. Neither path blocks its HTTP response/poll cycle on the sync, and neither touches `lib/sync.js`'s own retry/backoff scheduler — a redundant regularly-scheduled sync right after either of these just sees "already up to date". Closes #725
- **Shot backfill could get permanently stuck on a single missing/404 shot id.** A Gaggiuino machine's on-device shot storage rotates/caps independently of the monotonically increasing `lastShotId` counter it reports via `/latest` — a genuine 404 for one old shot id used to throw out of `syncShots()`'s backfill loop entirely, and since the resume point (`effectiveMax`) only advances when a shot is actually stored, every subsequent sync attempt restarted at that exact dead id forever, silently never syncing anything past it (looked like "sync is just broken" to the user). The backfill loop now catches per-shot fetch errors individually: a 404 blocklists that id (the same mechanism already used for user-deleted shots) and continues past it, while any other error (network/timeout/5xx) still aborts the whole sync attempt exactly as before, letting the existing retry/backoff schedule handle it. Also fixes a related logging gap: a failing shot id was previously invisible in logs (the error path's URL redaction stripped the id along with the host), now always logged explicitly alongside the error. Closes #721
- **A shot id outside the default machine's own range could permanently stop its sync from ever fetching another shot.** `ShotRepository.upsert()` defaulted `machine_id` to `1` whenever a shot was saved without an explicit `machineId` — if a shot ever carried another machine's synthetic id (`machineId * 10,000,000 + nativeId`, see `lib/machines/index.js`) through that path, it silently landed under the default machine anyway. `lib/sync.js`'s "already caught up" check (`syncShots()`) is a plain `max(local ids) >= latest-from-machine` comparison, so one such row was enough to make every future sync cycle log `Already up to date` and never pull another shot from the machine again — no error, no visible failure, indistinguishable from genuinely being caught up. `upsert()` now infers ownership via `ownerOfShotId(id)` instead of blindly defaulting to `1`, and `syncShots()`/`syncAfterBrew()` additionally ignore any id `>= MACHINE_ID_OFFSET` when computing the default machine's own max — a second, independent guard against a row that was already corrupted before this fix. Closes #719
- **Settings → Machines' host field can now be left empty — a machine can be saved in an explicit "not configured yet" state**, instead of being forced to hold a value. When no host resolves anywhere (registry row empty and `options.json`'s `machine_host` empty, the latter always true post-#662), `getMachineUrl()`/`getMachineBaseUrl()` (`lib/data.js`) now return `null` instead of silently falling back to a hardcoded placeholder hostname — every poll/sync call was previously attempting a real network request against that literal string whenever nothing was actually configured. `lib/poll.js`'s `pollViaGaggiuinoStatus()` and `lib/sync.js`'s `syncShots()`/`fetchMachineVersion()` now skip cleanly (no request, no error-state noise) when `null`. A fresh install's default machine now seeds with an empty host rather than a fake-looking one. Malformed-but-non-empty host input (bad scheme, unparseable) is unaffected, still on its existing fallback (#699). Closes #718

### Added
- **Debug-gated raw `/latest` response body from the machine** (Settings → `debug_logging`): logs the machine's actual response verbatim (truncated to 500 chars) right after fetching it, instead of only ever showing the value already extracted into local DB state — lets a genuinely corrupted/absurd `lastShotId` reported by the machine's own firmware be told apart from something produced locally. Came up chasing an implausible shot count (`Already up to date. Shots: 900000012`) that only reproduced on one specific install. Closes #717
- **Debug-gated per-shot response time during backfill** (Settings → `debug_logging`): each shot fetch during a sync now logs its elapsed time alongside the URL, e.g. `GET http://192.168.2.100/api/shots/247 -> 312ms` — makes the "does the machine's own embedded HTTP server slow down as shot history grows" question directly answerable from one log capture (plot/eyeball latency against shot id) instead of staying a guess. Closes #716
- **Debug-gated full request tracing for machine-facing calls, plus a registry snapshot on every change** (Settings → `debug_logging`): `lib/poll.js`'s status poll and `lib/sync.js`'s shot fetches now log the exact URL of every request, not just failing ones (#709 already covered those) — a wrong/stale host that still resolves to *something* was otherwise invisible until it happened to fail outright. Separately, `registry.logRegistrySnapshot()` dumps the full machine list (id, name, host, default flag, enabled) at startup and after every add/update/delete via Settings → Machines — a mismatch between a machine's registry id and which row is actually flagged default (e.g. after repeated add/remove cycles) was previously only reconstructible by reverse-engineering a synthetic shot id. Closes #714
- **`Machine added`/`Machine updated` log lines now include the host that was actually saved** (`Machine updated: #1 host=192.168.2.100`), instead of omitting it — during a live support round there was no way to confirm from the log alone whether a host change via Settings → Machines actually took effect. Closes #713
...
### Added
- **Debug-gated periodic connectivity summary for the machine host** (Settings → `debug_logging`): every ~60s, one aggregate log line built from the existing 1s status poll's own outcomes — e.g. `Connectivity (last 60s): 45/60 ok, avg latency 82ms, errors: EHOSTUNREACH x10, timeout x5`. Diagnoses a flaky/marginal link to the machine from the log alone, instead of relying on a user running `ping`/`curl` by hand from the right host (this support round needed several rounds of back-and-forth just to get that right). Closes #710
...
### Added
- **`GET /api/status`'s `machines[]` array now includes each machine's `theme`** (already stored by the registry and returned by `GET /api/machines`) — groundwork for `glp-integration`/the Lovelace and Order cards to sync their accent color to the app's own Settings → Machines theme picker instead of only their standalone YAML-config fallback. App-side only in this change; the cards don't consume it yet. Part of #701.
- **`machine_host`/`switch_entity` removed from the add-on's Configuration page — the default machine now configures entirely in-app, under Settings → Machines, the same way every additional machine already does.** These two options bridged the HA add-on config and the app's own machine registry via a tracked-input reconciliation layer (`lib/machines/options-adoption.js`) that caused five separate bugs from the same root cause (#638/#641/#643/#648/#661) — most recently a backup restore silently reintroducing a stale host that survived every subsequent restart. Soft deprecation, not a hard cutover: `config.yaml`'s `schema`/`options` no longer expose the two fields, but the backend bridge itself is untouched — an upgrading install's already-configured values are carried over automatically (no data loss, no manual steps), and a new one-time dismissible banner points at Settings → Machines for any install that still has an unconfirmed legacy value sitting in its (now frozen) add-on options, disappearing for good the moment that value is edited there. `glp-integration` needed no changes — it was already discovering the add-on purely via Supervisor's slug/hostname/port APIs, never the add-on's own option contents. Closes #662

### Changed
- **`buildGrinderStats()` migrated onto the shared `_computeEquipmentStats()`/`_renderEquipmentStats()` helpers** basket/puck-screen stats already use (#668), instead of duplicating the same grouping/aggregation/card-rendering logic a third time. `_computeEquipmentStats()` is now generalized to accept a `getKey`/`getName` pair — an id+library lookup for basket/puck screen, or an identity function for grinder's already-a-name free-text key — instead of a fixed `(library, idField)` shape. Also fixes a small drift the generalization surfaces: grinder's `avgDuration` used to be a string (`.toFixed(1)`), while basket/puck screen's was always a number (`Math.round(x*10)/10`) — both render identically today via template-literal coercion, but any future programmatic consumer would have seen inconsistent types. Pure refactor, no visible behavior change. Follow-up from #669's code review, flagged but not blocking at the time. Closes #674
- **`firmware-check.js`'s GitHub releases lookup now paginates instead of only checking the first ~30 releases.** If one release channel (`main-*`/`dev-*`) is published much more often than the other, the first page could be entirely the wrong prefix, making a real release look like "unknown" instead of being found. Now scans up to 5 pages, stopping as soon as a page has a matching-prefix release (GitHub returns releases newest-first, so the first match found while paging is still the true latest one) — bounded to protect the existing hourly-cached 60/hr unauthenticated rate-limit budget. Follow-up from #669's code review, flagged but not blocking at the time. Closes #673

### Fixed
- **Basket and Puck Screen showed "No basket"/"No puck screen" for the initially-selected shot on first page load**, even when both were correctly saved — navigating to a different shot and back always showed the real values. `main.js`'s init sequence fired `loadLibrary()` unawaited right before `loadData()` resolved and rendered the annotation panel; `renderAnnotationPanel()` reads `S.coffeeLibrary.baskets`/`.puckScreens` to resolve the id → name lookup, which was still empty on that first render. Same root cause class as the `shotDefaults` ordering fix already documented one line above it in `main.js` — `loadLibrary()` just hadn't gotten the same treatment. Now awaited before `loadData()`. Closes #700
- **Shot sync silently stopped advancing whenever a machine's host was configured *with* a scheme** (e.g. `http://192.168.1.50/`, the exact format the Machines "Add machine" dialog and the legacy `machine_host` option both accept/imply) **while live status polling and the live shot graph kept looking completely normal.** `lib/data.js`'s `getMachineUrl()` only appended `/api/shots` when the raw host had no `http(s)://` prefix; with a scheme present it was returned as-is, so `lib/sync.js`'s `syncShots()` requested `http://192.168.1.50//latest` instead of `http://192.168.1.50/api/shots/latest` — either hitting the machine's own SPA fallback (no error, sync just silently stopped advancing) or a genuine 404, with nothing pointing at host format as the cause. `getMachineUrl()` now always parses the input via `URL`, discards any path/query, and rebuilds as `${protocol}//${host}/api/shots` — the accepted host format no longer matters. Closes #699
- **The Live tab's flow reading/chart line was always flat at 0**, on both the WebSocket and MQTT live-data transports. `lib/machine-state.js`'s `deriveMachineState()` correctly read `sensorSnap.pumpFlow` into `machineStatus.pumpFlow`, but never returned it as a top-level value; `lib/poll.js`'s live-shot accumulator only ever destructured `pressure`/`temperature`/`weight` off that return value and hardcoded `pumpFlow.push(0)` — a placeholder left over from before either live transport existed, never wired up once `sensorSnap.pumpFlow` became available. Not a regression from the recent MQTT work specifically, but only became visibly noticeable once pressure/temp/weight started reflecting live sensor pushes and flow stayed flat by comparison. Closes #698
## [2.31.0] – 2026-08-07
...
- **Firmware update-availability check (#620 Phase 1, backend groundwork — no UI yet).** New `GET /api/machine/firmware/version` alongside the existing `firmware/progress`/`firmware/update` proxy (#597/#599): compares the machine's installed commit-hash version (`GET /api/settings/versions`) against the latest matching release on the firmware's own GitHub project ([`Zer0-bit/gaggiuino`](https://github.com/Zer0-bit/gaggiuino), not the docs-only `gaggiuino`/`GAGGIUINO` org), filtered by the machine's configured `system.releaseChannel` and cached hourly to stay well under GitHub's unauthenticated 60 req/hr rate limit (`lib/machines/gaggiuino/firmware-check.js`). Returns `{ installed, latest, updateAvailable, releaseUrl }`. **Documented, unverified assumption** (flagged explicitly per the issue, not silently treated as confirmed): firmware releases use commit-hash tags rather than semver (`main-<hash>`/`dev-<hash>`, no GitHub prerelease-flag distinction) — `releaseChannel` 0 (stable) and 1 (test) are both mapped to the `main-*` tag line, 2 (debug) to `dev-*`. This has not been confirmed against the OTA-trigger firmware source or a firmware maintainer; if wrong, the comparison could report the wrong update-availability for the test/debug channels. **A second, separate documented assumption**: `updateAvailable` is a raw string comparison of `installed` vs. `latest` with no format/length/case normalization — verified to match exactly (both short, lowercase hex) on the one real machine checked during research, but not guaranteed for every firmware build; if the two ever differ only in case/truncation for the same actual commit, this reports a phantom update indefinitely (never the reverse). Phase 2 (a `glp-integration` HA `update` entity consuming this) is explicitly out of scope for this round — see the PR description. Closes #620
- **Analytics: "Basket Stats" and "Puck Screen Stats" — score/shot-count groupings by basket and puck screen**, alongside the existing Grinder Stats section, same card layout and score-color coding. Baskets/puck screens are pure ID-based library links (`annotation.basketId`/`annotation.puckScreenId`, #635) — previously purely informational, now aggregated the same way Grinder Stats already groups by `annotation.grinder`. Reuses the existing `calcShotScore()` unchanged (no scoring-formula change) and the existing `bean-cards` rendering style. No schema change — the fields were already being recorded. Closes #668
- **The bottom-left status footer now shows how long the machine has been on ("on 2h 14m") instead of the last shot-sync clock time, while it's on.** Reuses `runtime.switchOnAt` (`lib/machine-runtime-state.js`), already tracked for `lib/preheat.js`'s elapsed-time math, exposed additively via `GET /api/status` as `machineOn`/`machineOnSince` — no new tracking added. Falls back to the previous last-sync display whenever the machine is off. Closes #681
- **Swipe from the left screen edge to open the mobile burger menu**, the mirror gesture to the existing swipe-left-to-close on the drawer itself — feels closer to how edge-swipe navigation drawers work on mobile OSes. Closes #682
- **A persistent yellow "⚠ UNSTABLE DEV BUILD" banner across the top of the app on dev-channel builds**, alongside the existing subtle version-badge suffix — much harder to miss that you're not on a real release. Non-dismissible, stacks correctly above the existing update-available and machine-unreachable banners. Closes #683

### Fixed
- **Settings → "Shot logging defaults" (#654): the Grinder field had no autocomplete, unlike the real shot annotation panel's equivalent field**, making it a plain guess-the-spelling text box instead of a pick-from-your-library list. `#annGrinder` already had `attachAutocomplete(el, () => S.coffeeLibrary.grinders.map(g => g.name))` wired up; the defaults card's `#sdGrinder` never got the same treatment when it was added. Closes #691
- **The dev-build banner (#683) sat on top of the topbar/menus instead of pushing them down**, hiding the nav entirely on GLP DEV rather than just occupying its own strip at the top. `body` is `height: 100vh; overflow: hidden` with global `box-sizing: border-box`, so the fixed banner now sets `body.style.paddingTop` to its own height on creation — the flex layout's available height shrinks by that amount instead of the banner overlaying whatever was already at the top. Closes #686
- **Restoring a backup left library/machines/settings/menu/etc. stale until a manual page reload.** The restore confirm handler (`public-src/components/backup-modal.js`) only ever called `window.loadData()` (shots only) before showing the result alert. Now reloads the page once after the alert is dismissed, so everything imported by a scoped or full restore renders immediately. Closes #684
- **The barcode/QR scanner (Library → ⬛ Scan) never worked — black camera preview, immediate `scan_error`, on every install.** `server.js` sends `Permissions-Policy: camera=(), microphone=(), geolocation=()`, added in an earlier security-hardening round (#176) with an empty camera allowlist — disabling camera access for the page entirely, same-origin included, so `getUserMedia()` was refused by the browser before any scanning code ran. The scanner feature (#70) shipped afterward and was broken from day one; confirmed live (zero backend log entries for `/api/library/scan/*` despite repeated scan attempts, proving the failure was purely client-side). Now `camera=(self)` — microphone/geolocation stay disabled, still unused. Closes #677
- **Stopping the add-on (app or dev) from the Supervisor UI always reported an error instead of a clean stop.** `server.js` never registered a `SIGTERM`/`SIGINT` handler. The Node process runs as PID 1 inside the container (`docker-entrypoint.sh` execs into `gosu node`, which replaces itself), and Linux does not apply the default signal disposition to PID 1 unless a handler is explicitly installed — so `docker stop`'s SIGTERM was silently ignored, Supervisor always hit the stop timeout, and Docker force-killed the process (exit code 137), which Supervisor then reported as add-on `state: error` rather than `stopped`. `server.js` now closes the HTTP server and exits cleanly on `SIGTERM`/`SIGINT`, with a short fallback timer in case `server.close()` hangs on an open keep-alive connection. Closes #675

### Changed
- **Consolidated two copy-pasted helpers into their canonical single-source-of-truth files, the same precursor shape as #638/#641/#643/#648 (machine host/switch entity resolution copy-pasted into five files, each accumulating its own bug).** `resolveMachine(rawId)` was duplicated verbatim in `routes/system.js` and `routes/machine-control.js`; `requireSettingsProxySupport()` was reimplemented inline in `routes/mqtt.js` instead of importing the canonical version. `resolveMachine()` now lives in `lib/machines/registry.js` (alongside `hostFor()`/`switchEntityFor()`/`baseUrlFor()`/`apiUrlFor()`); `requireSettingsProxySupport()` is exported from `routes/machine-control.js` for `routes/mqtt.js` to reuse. A new `eslint.config.js` `no-restricted-syntax` rule blocks either from being re-declared anywhere else, so this can't silently re-diverge in a future round. Pure refactor, no behavior change — all existing tests pass unchanged. Found during a proactive tech-debt sweep, not a live bug. Closes #679
- Internal: dev-channel build version format and changelog-path handling adjusted (CI/tooling only, no user-facing change). Closes #687, #689

## [2.30.0] – 2026-08-06

### Added
- **Backup export now downloads as a real `.zip` (`backup.json` + the actual image files) instead of a single JSON file with every photo base64-embedded inside it.** Investigating #657's missing-photos bug surfaced a deeper problem: re-encoding every shot/bean/grinder/basket/puck-screen photo as base64 inside one JSON blob meant an unopenable-without-the-app archive, ~33% size overhead per photo, and — the actual motivation — a whole extra encode/decode step where a bug like #657 could silently drop files. The zip now carries `backup.json` (unchanged shape, minus the embedded `images` field) plus one `images/<filename>` entry per photo, openable in any normal file browser. A new dependency-free `lib/zip.js` (local file headers, CRC32, `zlib.deflateRawSync`/`inflateRawSync` — the same "no new dependency" choice `lib/backup-crypto.js` already made for AES-256-GCM) reads and writes the archive; no archiver/adm-zip/yauzl pulled in. **Restore accepts both formats** — every backup already downloaded before this change (`.json`, base64 images) keeps restoring exactly as before; `POST /api/restore` now branches on `Content-Type`, with the zip path carrying `sections`/passphrase/dry-run as `X-GLP-Sections`/`X-GLP-Passphrase`/`X-GLP-Dry-Run` headers (never a URL query string — same reasoning already documented for why the export passphrase can't go in one) rather than inside the binary body. `GET /api/backup` is untouched, still the legacy self-contained JSON export for any existing bookmark/tooling.
- **Selective backup/restore, an encrypted API-token/MQTT-login section, and a dry-run preview before restoring — filed as #653 shortly after the backup-completeness fix below shipped, and built directly against that request.** ⚙ Settings → Backup & Restore now opens a modal instead of downloading/restoring unconditionally:
  - **Selective sections.** Six independently selectable domains — Shots & library, Maintenance, Orders, Machines, Settings, and the new encrypted secrets section below — usable on both export and restore. A scoped export only contains the sections you picked; a scoped restore only touches those sections and leaves everything else in the database exactly as it was, instead of the previous all-or-nothing wipe-and-replace. `POST /api/backup`/`POST /api/restore`'s `sections` field carries the selection; the plain `GET /api/backup` stays the unscoped, all-sections export for any existing bookmark/tooling. Restoring a scoped export without re-picking sections falls back to the sections it was scoped to on export, so re-uploading a "Maintenance only" file doesn't silently expand to "everything."
  - **API token and MQTT broker credentials, encrypted with a passphrase you choose.** Both were excluded entirely from every export until now — but the API token grants full API access (including this very restore endpoint) and has no "regenerate" button, so restoring from a backup without it meant re-pairing the HA integration and every direct-LAN client by hand. They're now available as an opt-in seventh section: pick a passphrase at export time (AES-256-GCM, scrypt key derivation, Node's own `crypto` module, no new dependency — `lib/backup-crypto.js`), enter it again at restore time. Nothing else in the backup is encrypted or requires a passphrase; GLP never stores the passphrase anywhere, and a forgotten one only costs you those two fields, not the rest of the backup. A wrong or missing passphrase at restore time never fails the whole operation — every other selected section still restores, and the response reports whether the secrets specifically came back so the UI can tell you plainly.
  - **A dry-run preview before anything is written.** The same restore endpoint accepts `dryRun: true` and runs every validation/sanitizer step a real restore does — counting how many shots/maintenance rows/orders/machines would actually be applied (and how many would be dropped as invalid) and whether the given passphrase can decrypt the secrets section — without touching the database or filesystem. The modal now shows this preview before you confirm, instead of the old blanket "all current data will be overwritten, continue?" `confirm()` dialog.
  - New `public-src/components/backup-modal.js` drives both flows from one implementation, so the section list and its labels can't drift apart between export and restore the way `resolveSwitchEntity()` drifted into five copies (#638–#648) — the same reasoning `lib/machines/options-adoption.js` documents for tracked options. `backup_desc` and the modal's own strings are translated in all 6 languages.
- **Backup filenames now include the time, not just the date** (e.g. `glp-backup-2026-08-06_08-32-05.zip`). Two backups taken the same day previously collided on the same `glp-backup-YYYY-MM-DD` filename, forcing the browser to append "(1)"/"(2)" or silently overwrite the earlier one. Closes #660

### Fixed
- **Hardened `lib/zip.js`'s `readZip()` and the barcode-scan proxy route with explicit local guards, closing 3 critical CodeQL findings surfaced when this release merged into `main`.** Both were assessed as false positives — `readZip()`'s only call site already checks `Buffer.isBuffer(req.body)` before calling in, and the barcode-scan route's request host is a hardcoded constant with only a schema-validated digit-only path segment coming from user input — but CodeQL's static analysis doesn't reliably connect a guard in a different function/file to the use site it flags, the same class of false positive this repo hardened against before (see `4efaa94`). Each site now re-asserts its own guard locally, redundant with the existing checks, so both the analysis and any future reader can see the barrier without needing interprocedural reasoning. Closes #665
- **The status dot could stay red for several minutes after the machine was switched on and was already reachable again**, because the one-off sync attempt fired 2s after "machine on" could fail if the physical machine's own HTTP API hadn't finished booting yet — and that failure's `lastSyncError` then sat untouched until whatever was left of the independent, unrelated 5-minute sync schedule. Found live on GLP DEV right after the fix above: `machineReachable` recovered within ~15s (the 1s reachability poll), but the dot stayed red for about 4 minutes. The post-power-on sync now retries a few times a short interval apart (bounded, well under a minute total) instead of a single fire-and-forget attempt, without touching the independent retry-backoff loop `lib/sync.js` already runs on its own schedule. Closes #663
...
- **Pressing Enter after typing a passphrase in the backup export modal did nothing — only clicking "Herunterladen" worked.** `#backupModal` (`public-src/index.html`) isn't a `<form>` (deliberately, to avoid an accidental submit-navigation from any focused field given the section checkboxes/preview wiring), and its buttons are `type="button"`, so Enter had no default action to fall back on. Enter in any of the modal's text/password inputs now clicks the confirm button, same as a real form submit would. Closes #656
- **Backup export could silently include only a fraction of the actual shot photos on disk, with no way to tell why.** A real export had 39 shots with a saved photo, but the backup's `images` block contained only 1 of them. `buildBackupBundle()`'s scan of `BEAN_IMAGE_DIR` (routes/backup.js) swallowed every per-file read error silently, so a file that failed to stat/read for any reason (e.g. a permissions mismatch) just vanished from the export with nothing in the log to distinguish it from "there was only ever one photo." Failures are now logged with the filename and error, so a future occurrence is diagnosable instead of an invisible gap. Closes #657
- **Shot photos (e.g. latte-art photos attached to a shot's notes) rendered correctly in the app but were never included in a backup export at all, even though bean/grinder/basket/puck-screen photos already were.** Found while re-testing the images-in-backup fix above: the export derived its file list from a hand-maintained table of "which library entity types can carry a photo" (`beans`/`grinders`/`baskets`/`puckScreens`), and shots — which store their own photo directly on the shot rather than nested under the coffee library — were simply never in that table. `GET`/`POST /api/backup` now read `BEAN_IMAGE_DIR` directly instead: every file actually on disk is included, regardless of which entity type claims it, so this whole class of "forgot to add a category" bug can't recur even for a future photo-bearing entity type. Restore gained the matching half — shot photos now validate and write back with the exact same magic-byte/size/id-derived-filename checks (never taken from the backup file itself) already used for library entity photos, closing the same path-traversal guard for this fifth category.
- **Checking "API token & MQTT login" and entering a passphrase in the backup export modal silently produced a file with no secrets in it at all.** The passphrase was sent and the backend correctly computed the encrypted `secrets` block — but the checkbox itself (`public-src/index.html`) was missing the `class="backup-section-cb"`/`value="secrets"` attributes `checkedSections()` (`public-src/components/backup-modal.js`) reads to know what's selected, so it was invisible to that collector regardless of whether it was checked. `sections` therefore never included `"secrets"`, and the backend correctly (but silently) dropped the block it had just built. Also fixed a smaller instance of the same class of bug found while reviewing this: the rail-footer status dot mirrored the main status dot's color but not its tooltip text when the machine went unreachable (#655, below), so it showed the right color with a blank hover title.
- **Opening the restore modal and toggling a couple of section checkboxes could hit "Rate limit exceeded" before the Restore button was ever clicked.** The new dry-run preview (fires on every section-checkbox toggle and debounced passphrase keystroke, see `#653`'s entry above) shared `POST /api/restore`'s rate limit with real, destructive restores — 3 requests/minute, sized for how often someone actually wipes and replaces their database, not for live UI feedback. A handful of normal interactions with the modal could exhaust that budget on read-only preview traffic alone. Dry runs now draw from their own, more generous limit (30/min, matching the pattern `routes/library/scan.js` already uses for similar preview-style traffic); real restores keep the original 3/min cap unchanged.
- **The status dot (top and rail) stayed green for hours or days after the machine was switched off, and the Live tab kept showing "Ready to brew" long after shutdown.** Reported as #655: "status dots stay green even though the machine has been off for hours/days; Live tab shows Ready to brew 10+ minutes after power-off; switching from mDNS hostname to a fixed IP made no difference." Three compounding causes, all in the reachability plumbing rather than in reachability detection itself (`lib/poll.js`'s 1s status poll and its 3s axios timeout were already correct): (1) the status dot (`public-src/components/status.js`) never read `machineReachable` at all — only `lastSyncError`/`lastSync`, which `lib/sync.js`'s `syncShots()` never touches when a configured HA switch entity reports the machine off, since its early return happens before any network call; (2) `GET /api/live/data` (`routes/system.js`) never returned `machineReachable` either, so a powered-off machine (`state.liveAccum === null`) was indistinguishable from an idle-but-reachable one, and the Live tab fell into its "ready" fallback; (3) the deeper root cause — `state.machineReachable` itself froze at its last value once the machine was switched off, because `lib/poll.js`'s `checkAndApplyMachinePower()` stops the only frequent reachability prober (`pollViaGaggiuinoStatus`, via `stopLivePolling()`) without ever setting `state.machineReachable = false` itself, and the mDNS→IP hostname change in the report never had a chance to matter because that code path was never reached. Fixed at all three points: the status dot now treats `machineReachable === false` as its highest-priority signal (still deferring to `lastSyncError` when the machine is reachable but a sync fails for other reasons); `/api/live/data` now includes `machineReachable`, and the Live tab renders a new explicit "unreachable" state (red badge/icon, translated in all 6 languages) instead of falling back to "ready"; and `checkAndApplyMachinePower()` now sets `state.machineReachable = false` itself the moment the switch entity's own "off" report is seen, the same signal `syncShots()` already trusts to skip its network call — no separate stale-timeout mechanism needed. `syncShots()`'s switch-off early return deliberately still leaves `lastSyncTime`/`lastSyncError` untouched: since the dot is now driven by `machineReachable` first, an old "last synced" timestamp next to a correctly red dot is accurate, not misleading.
- **`GET /api/backup` exported only 5 of the app's 10 database tables, so `POST /api/restore` silently threw away everything else.** Found while testing a real backup taken from a production install (172 shots, 8 beans) on a fresh instance: the maintenance page came back with every task showing "never done" and an empty log, despite the source install having a maintained history. `maintenance`, `maintenance_log`, `orders`, `machines` and the `kv` settings table (order menu, notification routing, import preferences, MQTT transport settings) were never part of the export at all — restoring a backup silently discarded all of it with no warning. Two further gaps surfaced during the fix: (1) `trash` *was* exported but `POST /api/restore` never read it back, and the trashed shots' own data was missing from the `shots` array in the first place, so the recycle bin was unrecoverable even though the backup file looked like it contained one; (2) bean/grinder `image` fields only ever stored a file extension (e.g. `"jpg"`), never the image file itself, so a restored library pointed at photos that had never been backed up and rendered as broken/blank thumbnails instead of a clean "no photo". All five now round-trip: images are embedded as base64 (validated against magic bytes and a strict id-derived filename on restore — a filename is never taken from the backup file itself, closing off path traversal), machines restore with their original ids so the `machine_id` columns other restored rows reference stay valid, and MQTT broker username/password are deliberately *excluded* from the export (this JSON file routinely ends up in Downloads/cloud backups; restoring one never overwrites locally configured MQTT credentials either). **Existing backup files made before this fix do not contain this data and cannot be used to recover it** — the maintenance log, orders, machine configuration, app settings, trash bin and library photos of a pre-fix backup are gone if the only copy was that file; take a fresh backup after upgrading. Old-format backups still restore cleanly (verified against the actual file from the report above) since every new field is read as optional, exactly like `coffee_library` already was. `backup_desc` updated in all 6 languages to describe the real scope instead of the old shots/library/notes-only wording.
- **`lib/sync.js`'s `fetchMachineVersion()` was the one remaining place still resolving the machine host straight from `options.json` instead of the registry** — missed by #641/#648, which fixed every other host call site. It's called every 30s from `backgroundHaCheck()` and sets `state.machineReachable = false` on failure, so a machine correctly rehosted via Settings → Machines could still be shown as unreachable. Now goes through the same registry facade as everything else (see below).
- **Introduced a single config facade for machine host/switch-entity resolution (`registry.hostFor()`/`switchEntityFor()`/`baseUrlFor()`/`apiUrlFor()` in `lib/machines/registry.js`) and removed every hand-written copy of the resolution logic it replaces.** #638, #641, #643 and #648 were four separate bugs with the same root cause: an `opts`-shaped signature (`getMachineUrl(loadOptions())`) that looks correct at every call site even when reading straight from possibly-stale `options.json` is the wrong thing to do. #643 alone had the same three-line `resolveSwitchEntity(opts)` copied into five files (`routes/system.js`, `lib/preheat.js`, `routes/orders.js`, `lib/poll.js`, `lib/sync.js`); #641/#648 added `resolveMachineHost(opts)` variants on top. All of those (plus `lib/data.js`'s `loadOptions()`, previously duplicated a second time in `lib/machines/registry.js`) are now one implementation each. Semantics preserved exactly: `hostFor`/`baseUrlFor`/`apiUrlFor` prefer the registry's host, falling back to `options.json` only when the registry has no usable host; `switchEntityFor` returns the registry's value even when it's `null` (an explicitly cleared field means "not configured", #643) and falls back to `options.json` only when there's no default-machine row at all. `lib/data.js`'s `getMachineUrl()`/`getMachineBaseUrl()` stay as the underlying URL normalizers the facade wraps — no longer imported directly outside of the facade itself. A new ESLint rule (`no-restricted-syntax` on `opts.machine_host`/`opts.switch_entity` property access, `eslint.config.js`) makes the old pattern a hard lint error everywhere except the three files that legitimately read `options.json`. New `test/settings-take-effect.test.js` covers the facade directly (including the #643 "cleared registry value must not fall back to options.json" case) plus the consumers that had no live-registry test coverage yet (`fetchMachineVersion()`, `GET /api/switch`, `POST /api/switch/toggle`, `GET /api/status`'s `switchEntity`/`machineUrl` fields). `routes/system.js`'s `GET /api/status` also now resolves its `machineUrl`/`machineHostname` display fields through the registry — the only call site of this class actually found still reading `options.json` unconditionally, cosmetic (display-only, no functional gating) but the same bug shape as #638. Local refactor, not released as its own version bump.
- **A switch entity configured in the Home Assistant add-on options never reached the app if it was set after the add-on's first run.** Reported by a user on v2.29.0: "configuring the switch in the HA add-on configuration wasn't changing anything, but once I added my entity directly under Settings → Machines, the power switch popped right back up next to Sync." Two correct decisions combined into a gap: `registry.ensureDefaultMachine()` seeds machine #1 from `options.json` only while the machines table is still empty, so an option set later never reached the registry; and #643 then made every consumer read the registry, deliberately treating an empty registry `switchEntity` as "not configured" so that *clearing* the field in Settings actually sticks. Anyone who configured `switch_entity` in the add-on options after that first run was left with a NULL registry row that nothing would ever fill — no error, no log, just a power button that never appeared. A read-time fallback to `options.json` cannot fix this without re-breaking #643: it would resurrect the old value on every read and make the field unclearable. `options.json` is therefore now treated as a *tracked input* (`lib/machines/options-adoption.js`, run once at startup): `machine_host` and `switch_entity` are compared against the value seen on the previous start (stored in the existing `kv` table), and a **changed** add-on option is adopted into the default machine's registry row while an **unchanged** one is ignored — so an edit made in Home Assistant takes effect, and an edit made in the app (including a deliberate clear) always wins over an option that hasn't moved. On the first start after this upgrade there is no previous value to compare against, so the add-on option is adopted only into an empty registry field, which repairs exactly the stranded installs described above. `machine_host` is never cleared this way: an emptied add-on option means "leave it alone", not "forget the host", since the app would otherwise lose its only route to the machine. An option that is *absent* from `options.json` entirely is likewise never adopted, and is kept strictly distinct from one the user emptied: the Supervisor writes every key defined in `config.yaml`'s schema, so an absent key means the option was removed from the schema (the planned deprecation of `switch_entity`) or that there is no `options.json` at all (a standalone Docker install) — treating either as a deliberate clear would wipe the configured switch entity of every install on a single upgrade.
- **Form fields with suggestions (grinder, bean variety/process, grinder burr type, recipe profile/bean) rendered with the browser's native `<datalist>` UI, which looked visually inconsistent with the rest of the app and was unreliable to tap on Android/Chrome.** Reported by a Pixel/Android-Chrome user: "adding a grinder opens a somewhat different UI menu style compared to the rest of the app, and tapping it on mobile doesn't seem to work for me always." `<datalist>` is rendered entirely by the browser (a known cross-platform styling/tap-reliability limitation, not fixable from the input side), so all six affected inputs (`annGrinder`, `beanFormVariety`, `beanFormProcess`, `grinderFormBurrType`, `recipeFormProfile`, `recipeFormBean`) now use a new reusable in-app autocomplete component (`public-src/components/autocomplete.js`) styled with the app's own dropdown tokens instead. Free text stays valid at all times, exactly like `<datalist>` did. Selection reacts to `pointerdown` on a suggestion rather than `click` — the actual mechanism behind the Android bug: a plain click handler can miss because the input's `blur` (which would hide the list) fires before the click ever reaches the option; `pointerdown` with `preventDefault()` keeps the input focused so that race never happens. Full keyboard support (arrow keys, Enter, Escape) and combobox/listbox ARIA. The suggestion sources (`S.coffeeLibrary.beans`/`.grinders`, `S.machineProfiles`, the static variety/process/burr-type lists) are read live on every keystroke, so no separate "populate the list" step was needed to keep them in sync with library edits.
- **Tab bars (Coffee Library's 7 tabs, and the shot chart's Zeit/P·Q tabs) had no way to scroll on narrow screens, so the whole page scrolled sideways instead.** Reported by the maintainer for the mobile Library view. `.lib-tabs`/`.chart-tabs`/`.fs-tabs` were plain `display:flex` with no `overflow-x`, and since `#main` has `overflow-x:hidden`, a tab bar wider than the viewport dragged the entire page horizontally instead of just the bar. All three now share one new `.tabs-scroll` class (`overflow-x:auto`, hidden scrollbar, `-webkit-overflow-scrolling:touch`, non-shrinking tab children) instead of each bar needing its own copy of the fix.
- **Barcode scan (EAN/UPC → Open Food Facts lookup) was completely dead for every user.** `server.js`'s CSP pins `connect-src` to `'self'` (deliberate hardening — `config.yaml` advertises the app as "all local, no cloud"), so the frontend's direct browser `fetch()` to `world.openfoodfacts.org` was always blocked; the surrounding `try/catch` swallowed the failure and showed the same generic `scan_error` with nothing logged, so the feature just silently never worked for anyone. Added a backend proxy route (`GET /api/library/scan/:barcode`, `routes/library/scan.js`) that validates the barcode (8/12/13/14 digits only, via a new `scanBarcodeSchema`), applies the same `assertPublicHost()` SSRF hardening and byte-limit pattern the bean-import route uses, and returns only the fields the bean-import form needs. The frontend now calls this relative route instead — HA-ingress-safe — and the CSP itself stays untouched. The proxy also distinguishes "product not found" (404) from "lookup failed" (502), instead of one silent catch-all error.
- **Add-on log timestamps were hardcoded to German locale/Europe-Berlin time regardless of the container's actual timezone.** `lib/helpers.js`'s `log()` used `new Date().toLocaleString('de-DE', { timeZone: 'Europe/Berlin' })` unconditionally — for anyone outside Germany the add-on log (the primary support channel) showed times that couldn't be correlated with anything, in a comma-separated locale format that doesn't sort. Replaced with a new `formatLogTimestamp()` that reads the `Date` object's own local fields (which already resolve against whatever `TZ` HA/Docker set) and renders a fixed-width, sortable `YYYY-MM-DD HH:mm:ss`.
- **An unrecognized browser language silently fell back to German instead of English, both client- and server-side.** `public-src/state.js` set `S.currentLang` from `navigator.language` with no check against the languages GLP actually ships, and `i18n.js`'s `t()` fell back to `TRANSLATIONS.de` for any unsupported language — so a browser set to e.g. Portuguese, Polish or Swedish saw the entire UI in German, not English. `state.js` now validates against `TRANSLATIONS` and falls back to `en`; `t()`'s per-key fallback now also prefers `en` over `de`. The same bug existed server-side: `lib/ha.js`'s `getHaLanguage()` and `lib/notify-i18n.js`'s `notifyT()` both defaulted to `'de'`, so a standalone Docker install with no Supervisor token got German push notifications by construction — both now default to `en`. The ~14 call sites across `public-src/components/` and `public-src/views/` that each duplicated their own `LOCALE_MAP[S.currentLang] || 'de-DE'` for date/time formatting are consolidated into a single `localeFor()` helper in `public-src/constants.js`, which carries the same English fallback.

## [2.29.0] – 2026-08-04

### Added
- **Baskets and Puck Screens as new Coffee Library entity types, alongside beans/grinders/recipes/milks.** Two new Library tabs (grinder-style layout: list + edit/delete + photo upload) with enum fields — wall type/shape for baskets, thickness for puck screens. Shots can now be linked to a specific basket and puck screen the same ID-based way beans already are: a Basket/Puck-Screen select next to the bean select in the shot annotation panel, wired into the recipe card chip, CSV export, and `.shot` label export. Translated in all 6 languages (de/en/it/fr/es/nl). While implementing the restore-sanitizer wiring for the two new entities, also closed a pre-existing gap where milks were the only library entity never re-sanitized on `/api/restore` (beans/grinders/recipes already were). Closes #635

### Fixed
- **Editing the default machine's switch entity in Settings never actually took effect.** Same root cause as #638, one field over: `lib/poll.js`'s `checkAndApplyMachinePower()`, `lib/preheat.js`'s `buildPreheatResponse()`/`_checkReadyByPreheat()`, `lib/sync.js`'s `syncShots()`, `routes/orders.js`'s `_getPreheatInfo()`, and `routes/system.js`'s `/api/status`, `/api/switch`, `/api/switch/toggle`, `/api/preheat/ready-by` all resolved `switch_entity` straight from `options.json`, ignoring `registry.updateMachine()` writes entirely — so after editing the switch entity (or configuring it for the first time) in Settings, preheat-ready notifications, the Ready-by auto-switch-on, and the manual power toggle kept acting on the old (or no) entity. All of these now resolve `switch_entity` from `lib/machines/registry.js`'s `getDefaultMachine()` first, falling back to `options.json`'s `switch_entity` only when there's no default-machine row at all — an explicitly cleared registry value correctly means "not configured" rather than silently reverting to a stale `options.json` value. Closes #643
...
- **Hardened against a latent process-crash risk: an unguarded async rejection inside a 30s poller could have killed the entire backend, not just that poller.** An audit found `server.js` had no global `process.on('unhandledRejection', ...)` handler, and two `setInterval` callbacks (`lib/preheat.js`'s `startPreheatWatcher()` calling `_checkPreheatNotify`/`_checkReadyByPreheat`, and `server.js`'s `setInterval(backgroundHaCheck, 30000)`) called async functions bare, with no `await`/`.catch()`. Both were latent only — every awaited call inside those functions already has its own internal try/catch — but Node ≥15 terminates the whole process on an unhandled promise rejection by default, so a future change removing one of those inner try/catches would have silently killed the backend instead of just that poller. `server.js` now installs a global `unhandledRejection` handler that logs via the existing `log()` helper instead of crashing, and both interval callbacks now guard themselves with `.catch(err => log(...))`, mirroring the guard style `lib/poll.js`'s `pollLive`/`lib/sync.js`'s `scheduleNextSync` already use. No behavior change under normal operation. Closes #642
- **`shots/index.js`'s `loadData()` had no guard against overlapping calls.** It's fired unawaited from at least 6 independent triggers (init, the reload-data button, `live.js`'s post-brew `setTimeout`, `status.js`, `sidebar.js`, `onboarding.js`) with no coordination between them — if two calls overlapped and their responses arrived out of order, a slower earlier call's stale response could overwrite `S.allShots`/`S.shots` moments after a faster later call already wrote the current data, transiently reverting the sidebar/shot list (e.g. a just-finished shot disappearing) until the next poll or manual reload. Now uses the same monotonic request-generation-token pattern `loadMachineProfileList()` already established in `library-profile-editor.js` (#521): only the call that's still the latest one when its response lands is allowed to write state; a stale response is dropped silently. Closes #644
- **Four low-severity findings from the 2026-08-04 audit, bundled together since none warranted their own round.** (1) The dev-only `/api/debug/machine` endpoint (`routes/system.js`, gated behind `NODE_ENV !== 'production'`) still resolved the polled host straight from `options.json`, same stale-host bug #638 fixed elsewhere — now goes through the registry via a new `resolveMachineHost()` helper. (2) `routes/maintenance.js`'s `machineHostname()` had the same issue for the display/log-only hostname text stored on maintenance log rows — cosmetic (a host edit only affected which hostname string future log entries showed), same registry-first fix. (3) `views/library.js`'s bag-history block rendered `bg.roastDate` into `innerHTML` without `esc()`, unlike every sibling field there (`batchNumber` etc.) — not reachable via the UI (`type="date"` input), only via a crafted direct API call or a compromised import/backup-restore path; now wrapped in `esc()` for defense-in-depth consistency. (4) `views/analytics.js`'s world-map `countries-110m.json` fetch had no request-generation-token guard, unlike the pattern `library-profile-editor.js`/`shots/index.js` (#644) already use — added the same guard, low-impact since it's a static asset cached in `_worldTopo` after the first load. Closes #648

## [2.29.0] – 2026-08-04

### Changed
- **`scripts/dev-stats.mjs` (generates `DEVELOPMENT.md`) now reports the real Claude Pro subscription cost instead of a token-based estimate, and adds an hours-of-development estimate.** The old "Rough cost estimate" section multiplied changed lines in Claude-co-authored commits by an assumed tokens-per-line constant, then applied a price table (`scripts/dev-stats.pricing.json`) — modeling pay-per-token API billing that has no relationship to the flat-rate Claude Pro subscription actually used ($20/month, paid regardless of usage). The pricing file and all line/token-cost math are removed; the new "Claude Pro subscription cost" section is `monthsSinceStart × $20`, where a partial calendar month still counts as a full month. A new "Hours of development (lower-bound estimate)" section clusters each repo's commit timestamps into working sessions (commits within a 2-hour gap join the same session, each session gets a 30-minute lead-in credit ahead of its first commit — the git-hours heuristic), summed per repo and combined, and is explicitly labeled as a lower bound derived from commit timestamps only since long AI-agentic sessions can run for hours between commits. All three constants (`CLAUDE_PRO_MONTHLY_USD`, `SESSION_GAP_HOURS`, `SESSION_LEAD_IN_MINUTES`) live near the top of the file for easy tuning. Closes #623
...
### Added
- **MQTT as an alternative live-data transport, selectable in Settings ("Live connection: WebSocket / MQTT"), with Supervisor broker auto-discovery.** Builds directly on #597's WS live-state cache/event seam (`lib/gaggiuino-live-client.js`): a new `lib/gaggiuino-mqtt-client.js` subscribes to the Gaggiuino's own MQTT-published topics (`<prefix>/sensors`/`system`/`shot`/`profile/active`/`maintenance`/`notification`/`status`, per the machine project's own `MQTT.md`) and translates the `sensors`/`system` JSON payloads into the exact same field names `lib/gaggiuino-proto.js`'s `SensorStateSnapshotDto`/`SystemStateDto` decode WS pushes into — a new `lib/live-transport.js` dispatches every `GET /api/machine/live`/`GET /api/machine/status` cache read to whichever transport is active, so `glp-integration` and the rest of the app stay completely unaware of which one is running underneath (zero changes needed there). Broker connection details (`services: [mqtt:want]` in `config.yaml`) are auto-discovered from the HA Supervisor's `/services/mqtt` endpoint and pre-filled in Settings, with manual host/port/user/pass entry as a fallback for installs with no registered MQTT service; a one-click "Apply to machine" button points the Gaggiuino's own MQTT client settings at the same broker via the #597 settings proxy. MQTT only ever applies to the default machine — additional multi-machine-registry (#317) machines always keep using their own WebSocket session, since the broker/topic-prefix is scoped to one physical unit. Command writes and the machine's own native HA MQTT Discovery stay explicitly out of scope. Tested against a real in-process MQTT broker (`aedes`), not just mocked payloads. Closes #598
...
- **Gaggiuino settings/control API proxy (backend, no UI yet) — phase 1 of making `glp-integration` a full replacement for the community ALERTua/hass-gaggiuino integration.** New `/api/machine/*` routes (`routes/machine-control.js`) proxy the machine's own settings (`GET/POST /api/settings/*`), operation-mode/tare/service-test/save-active-profile commands, and firmware OTA endpoints, all Gaggiuino-only (`adapter.capabilities().settingsProxy`, 501 for GaggiMate). `lib/gaggiuino-proto.js` gained `UpdateSystemStateCommandDto`/`ServiceTestCommandDto`/`SensorStateSnapshotDto`/`SystemStateDto` (transcribed from the machine project's own published WS API doc) and `lib/gaggiuino-ws-client.js` a `sendCommand()` path that correlates by the generic `d_resp` ack instead of a specific push. A new `lib/gaggiuino-live-client.js` keeps one persistent WebSocket session per machine (reused across every poll, respecting the firmware's 3-connection limit) caching `d_sensor_snap`/`d_sys_state` pushes — exposed via `GET /api/machine/live` and merged into the default machine's `GET /api/machine/status` (`pumpFlow`, `weightFlow`, `waterTemperature`, boiler/valve states, thermocouple/pressure fault flags+reasons). That module emits `sensor-snap`/`sys-state` events on a shared bus as an observer seam for a possible future MQTT publisher (not built here). Live-verified against a real machine: entering `BREW_MANUAL` while idle is a silent no-op, so that mode is rejected by `/api/machine/opmode` rather than accepted and silently doing nothing. Closes #597

### Fixed
- **GLP wordmark and subtitle lines in `logo.svg` sat pushed toward the bottom of the banner instead of centered between top and bottom edges.** Closes #593
...
### Added
- **`POST /api/orders` now persists a client-supplied `beanId`.** glp-order-card#35 started sending a `beanId` field with each order for stable order-to-bean attribution, but the backend silently dropped it. `OrderService.placeOrder()` now resolves the id against the library's actual beans — a stale or fabricated id (deleted bean, or a value from a client with a stale cache) becomes `null` rather than failing order placement, mirroring the card's own name-fallback behavior. The field now flows through `POST /api/orders`'s response, `GET /api/orders` and `GET /api/orders/mine` unchanged. No new UI in this release — this makes the attribution data correct/stable so a future card release can use it. Closes #563

### Changed
- **Ecosystem-audit refactor batch, no behavior change (#548–#555):** `lib/state.js`'s ten machine-specific fields (temp, poll timer, stability, etc.) moved into a new per-machine `MachineRuntimeState` (`lib/machine-runtime-state.js`), separating them from the genuinely global fields that stayed on the singleton (#549). `routes/library.js` split into `routes/library/{beans,milks,grinders,recipes}.js` behind a single shared `express.Router()` (#550). The `lib/data.js` ↔ `LibraryService` require cycle is gone — `LibraryService` now calls `OrderRepository` directly instead of round-tripping through `lib/data.js` (#554). `routes/orders.js`'s domain logic (queue position/ETA math, accept/complete/decline lifecycle) moved into `OrderService`, unit-testable without HTTP round trips (#548). Raw SQL in `routes/backup.js` (trash lookup, four-table wipe) and `routes/maintenance.js` (log delete) moved into the repository layer (#553). `deriveMachineState()`/`isStillWarm()` extracted out of `lib/poll.js` into `lib/machine-state.js` as pure functions (#552). The bean-remaining-quantity calculation duplicated between the backend (`LibraryService.computeBeanRemaining`) and the frontend (`views/library.js`) is now shared via `public-src/bean-math.js` (#551). `test/import-generic.test.js` now runs against real fetched/archived Shopify snapshots instead of hand-written fixtures (#555). Closes #548, #549, #550, #551, #552, #553, #554, #555
- Added the `@vitest/coverage-v8` provider and a CI coverage gate (statements 41.21%/branches 38.37%/functions 33.68%/lines 43.87%, just below the measured baseline) so coverage regressions actually fail CI instead of only reporting a number nobody checks; local `npm test` stays coverage-free. Closes #556
- `CONTRIBUTING.md`'s code-notes table corrected to the current architecture: Vite build (not a Vite-less single file), `better-sqlite3` for shot data (not JSON-only storage), 6 UI languages (not 5), and the version source (`lib/constants.js`, not `server.js`). Closes #557
...
- **Added an ESLint 10 flat-config CI gate and a `scripts/release-check.mjs` sanity script for releases.** `npm run lint` is now wired into CI after `npm ci` and before `build`, with Node/browser/vitest globals split by path glob; baseline lint cleanup was mechanical fixes plus a few narrowly justified inline disables, no behavior change. ESLint 10 rather than 9 because 9's dependency tree carries a transitive high-severity DoS advisory (`brace-expansion`) that's only fixed in 10 — the flat-config API is identical between the two. `release-check.mjs` is a read-only, no-network script for manual release use, checking version-file consistency, the CHANGELOG heading, screenshot/dev-stats freshness, and DOCS.md/DOCS.de.md heading parity. Closes #519
...
### Removed
- **Dropped the in-app add-on self-update button entirely, reverting the unreleased #514/#515 fix rather than shipping it.** #514/#515 fixed a 403 on `POST /api/update` by granting the add-on `hassio_role: manager` in `config.yaml`. Revisiting that trade-off before release: `manager` isn't a narrow grant — per the Supervisor's role regex it also covers `/backups*`, `/core/*`, `/host/*`, `/os/*` and `/supervisor/*`, not just `/addons/*` — for a button whose functionality Home Assistant already provides natively via its own per-add-on Supervisor update entity (`update.<slug>_glp_update`), which never touches this app's own token or permissions. Kept the redundant, more-privileged in-app path only reduces security for no real gain, so it's removed instead: `config.yaml` no longer sets `hassio_role` (`hassio_api: true` stays — still needed for the harmless `GET http://supervisor/info` token-verification check, which is in the Supervisor's `api_bypass` allowlist and needs no elevated role); `routes/system.js` no longer exposes `POST /api/update`; the update-available banner's "Install now" button is gone from the web UI, replaced with a note that updates run through the HA Add-on Store (`GET /api/version` and the banner itself are unaffected — still shows when a newer version is out). Companion change in glp-integration (mxkissnr/glp-integration#54, mxkissnr/glp-integration#55) removes the update entity's install capability, keeping it as a read-only version display. Closes #516, supersedes #514/#515 (neither ever released).
...
### Added
- **Bean filter for the shot history sidebar.** Clicking a bean name in the Library now sets a structured filter (`S.beanFilter`, `sidebar.js`) that narrows the shot list to that bean's shots, shown as a clearable indicator above the list — ANDed with the existing free-text search rather than replacing it, the same way the machine filter already sits alongside search instead of competing with it. Matching goes through `resolveBeanForAnnotation()` first (the same beanId-with-name-fallback logic the header recipe card already uses) so shots predating `beanId`, or belonging to a bean since renamed, still match correctly; only when no library bean resolves at all does it fall back to a raw case-insensitive name comparison against the filtered bean's own name. Closes #466
- **Frozen/single-dose portion tracking on bean bags.** A bag can now have one or more portions moved into a dated frozen pool (`frozenPortions[]`, `POST /api/library/bean/:id/freeze-portions`) — grams and a portion count, still counted in the bag's `stock_g` and unaffected by freezing, since nothing is actually consumed. While a portion is frozen its share of the bag's roast-age clock is paused: `adjustedRoastAgeDays()` (`public-src/utils.js`) subtracts the accumulated frozen duration from the raw `roastAgeDays()` figure, so the freshness badge reflects genuine time-since-roast minus time spent below freezing. An explicit thaw action (`POST /api/library/bean/:id/thaw-portion`) stamps `thawedAt` and resumes aging for that portion from then on; thawed portions stay in the bag's history for an audit trail instead of being removed. Server-side sanitization (`sanitizeFrozenPortions()`, `lib/sanitize-bean.js`) caps portion count/weight to plausible single-bag ranges and drops malformed entries, mirroring how the other bean-field sanitizers already bound their inputs. Closes #466
- **README/DOCS clarification: a machine's `type` selects the firmware adapter, not the physical brand.** Documented that `type: gaggiuino` / `type: gaggimate` picks which WebSocket/REST adapter GLP talks to — it does not itself imply anything about the physical espresso machine's manufacturer — to head off confusion now that GaggiMate support exists alongside Gaggiuino. Closes #466
