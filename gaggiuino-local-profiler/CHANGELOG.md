...
- **Pressing Enter after typing a passphrase in the backup export modal did nothing — only clicking "Herunterladen" worked.** Reported by Max. `#backupModal` (`public-src/index.html`) isn't a `<form>` (deliberately, to avoid an accidental submit-navigation from any focused field given the section checkboxes/preview wiring), and its buttons are `type="button"`, so Enter had no default action to fall back on. Enter in any of the modal's text/password inputs now clicks the confirm button, same as a real form submit would. Closes #656
- **Backup export could silently include only a fraction of the actual shot photos on disk, with no way to tell why.** Reported by Max: a real export had 39 shots with a saved photo, but the backup's `images` block contained only 1 of them. `buildBackupBundle()`'s scan of `BEAN_IMAGE_DIR` (routes/backup.js) swallowed every per-file read error silently, so a file that failed to stat/read for any reason (e.g. a permissions mismatch) just vanished from the export with nothing in the log to distinguish it from "there was only ever one photo." Failures are now logged with the filename and error, so a future occurrence is diagnosable instead of an invisible gap. Closes #657
