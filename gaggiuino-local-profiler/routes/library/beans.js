...
// hop per non-matching sub-router in every request that falls through to a
// later-mounted router, which is invisible in production but hangs
// fake-timer tests elsewhere in the suite. See routes/library/index.js.
module.exports = function registerBeanRoutes(router) {
...
router.post('/api/library/bean', (req, res) => {
    ...
    const { name, roaster, roastDate, notes, stock_g, decaf, origin, origins, variety, species, category, process, flavors, roastType, region, imageUrl,
    ...
    const bean = {
        ...
        variety: s(variety, 200), species: sanitizeSpecies(species), category: sanitizeCategory(category), process: s(process, 200),
...
router.put('/api/library/bean/:id', (req, res) => {
    ...
    const { name, roaster, roastDate, notes, stock_g, decaf, origin, origins, variety, species, category, process, flavors, roastType, region,
    ...
    if (species !== undefined)   lib.beans[idx].species   = sanitizeSpecies(species);
    if (category !== undefined)  lib.beans[idx].category  = sanitizeCategory(category);
