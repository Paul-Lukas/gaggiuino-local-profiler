...
function _renderCustomSection(container, scope) {
...
function _renderDisabledSection(container, tiles) {
...
export async function setMaintMode(task, mode, machineId, currentShots, currentDays, currentG) {
...
export async function toggleMaintDisabled(task, machineId, disabled) {
  try {
...
export async function addCustomMaintTask(machineId) {
  const label = document.getElementById('customTaskLabel')?.value.trim();
  if (!label) return;
  const shots = document.getElementById('customTaskShots')?.value;
  const days  = document.getElementById('customTaskDays')?.value;
  const body = {
    label,
...
export async function renameCustomMaintTask(task, newLabel, machineId) {
