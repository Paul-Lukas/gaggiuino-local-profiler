...
export function findGrinderByName(grinders, name) {
...
export function currentGrinderZeroPoint(grinder) {
  const history = grinder?.zeroPointHistory;
  if (!Array.isArray(history) || !history.length) return null;
  return [...history].sort((a, b) => a.since - b.since).at(-1).zeroPoint;
}
...
function zeroPointAtTime(grinder, timestampMs) {
  const history = grinder?.zeroPointHistory;
  if (!Array.isArray(history) || !history.length) return null;
  let value = null;
...
export function normalizeGrindToNow(grinders, grinderName, value, shotTimestampMs) {
