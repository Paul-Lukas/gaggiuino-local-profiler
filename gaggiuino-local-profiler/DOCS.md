...
2. `GET /api/token` returns the token to any caller that can reach the port, rate-limited to 10 requests per minute per source address. See the trust model below for why (#533) — this is the default; the `expose_api_port` option can turn it off for non-Ingress callers.
...
**Notifications card (Settings, always visible).** Two of the app's automatic push notifications aren't Orders-specific — the machine finishing warming up and a bean's stock running low — so their on/off toggles live in a "Notifications" card on the main Settings page rather than inside the Orders admin panel, and work regardless of whether `enable_orders` is set. The other three notification types (shop opened/closed, new order to barista, order status to customer) stay in the Orders panel's "Push notifications" section, since they can't fire without Orders enabled anyway. Both locations read/write the same settings blob (`GET/POST /api/orders/settings`).
...
Each machine can be given its own **colour theme**, so machines in a multi-machine setup are easy to tell apart at a glance. Pick one of 8 curated presets (Amber Americano, Ruby Ristretto, Copper Cortado, Twilight Turkish, Marbled Macchiato, Ember Espresso, Mulberry Mocha, Frosty Flat White) or a custom flat colour or two-stop gradient from the machine form's swatch picker. The theme colours a detailed machine icon — rectangular display for Gaggiuino, round puck for GaggiMate — shown in the machine list, the machine form's live preview, and the topbar machine switcher; a machine with no theme set keeps rendering in the app's own accent colour.

When the **default machine** has a colour theme set, that theme also becomes the **whole app's accent colour** — Settings' "Farbschema" swatch picker is overridden live wherever it applies (buttons, highlights, active states), updating instantly if you switch the default machine or edit its theme, no reload needed. This is additive: the swatch picker keeps working exactly as before when the default machine has no theme, and its swatches stay clickable even while overridden (so your pick is still recorded for later) — just shown dimmed with a short note explaining why they currently have no visible effect. Only the default machine's theme drives the app accent; other machines' themes stay icon-only.

### Gaggiuino settings/control API proxy (backend, no UI yet)

Phase 1 of making `glp-integration` a full replacement for the community [ALERTua/hass-gaggiuino](https://github.com/ALERTua/hass-gaggiuino) integration (see the [compatibility table](#alertuahass-gaggiuino-compatibility) below): a Gaggiuino machine's own settings and control surface, proxied through GLP's API so a future round of `glp-integration` HA entities can consume it without talking to the machine directly. Gaggiuino-only (`adapter.capabilities().settingsProxy`) — a GaggiMate machine 501s on all of these. No GLP UI exposes any of this yet; that's a later round.
...
### Achievements (stamp card)

A dedicated **Achievements** view shows a printed-cardboard "stamp card": 54 badges across 7 categories (basics, craft, beans, endurance, care, house and a secret category), each unlocked automatically as you brew — no manual claiming. Unlocking is event-driven (checked right after a shot, maintenance entry or library change) plus a one-time retroactive pass over your existing history the first time the view loads, so upgrading doesn't leave earlier shots uncounted. Every badge field is pre-printed identically; only the stamp — a rough ink mark, slightly askew at a fixed angle per badge — distinguishes earned from unearned. Secret badges show only a "?" until unlocked; their name and description never reach the browser before that. A category with every badge earned gets a diagonal, dated "Full" overprint. There is no level, rank or point score — it's a keepsake, not a leaderboard. Achievements are per-install, not per-machine: multi-machine setups share one collection.
...
**Preview before restoring.** Before you confirm a restore, the modal shows exactly what would change — how many shots, maintenance rows, orders and machines would be applied (and how many would be skipped as invalid), and whether a given passphrase can actually decrypt the secrets section — computed by running the real restore's own validation, without writing anything yet. Once a restore completes, the page reloads automatically after you dismiss the result message, so everything you just restored shows up right away.

**API token and MQTT login, encrypted with a passphrase you choose.** Both are excluded from the backup by default: the API token grants full API access (including the restore endpoint itself) and the MQTT username/password are real broker credentials, and this file routinely ends up in Downloads or synced to cloud storage. Check the "API token & MQTT login" section and enter a passphrase to include them, encrypted (AES-256-GCM). You'll need the same passphrase to restore them — GLP never stores it anywhere, so if you forget it, the rest of the backup is still fully usable, only the token and MQTT login can't be restored from that file. A wrong or missing passphrase never blocks the rest of a restore.

**Older backups** (`.json` files downloaded before the ZIP format, and even older ones from before selective sections existed — see `CHANGELOG.md`) still restore normally; you don't need to re-export anything already saved. The very oldest `.json` backups (from before sections/photos/maintenance/orders/machines/settings were part of the export at all) contain shots, annotations, the coffee library and blocklist only — that older data was never captured and can't be recovered from those specific files. Take a fresh backup after upgrading if you rely on the newer sections.
